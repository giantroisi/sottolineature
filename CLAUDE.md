...
3. **Si pubblica solo cio' che e' completo.** Una citazione, un'opera o un autore entrano nel sito quando hanno **tutto**: testo verificato, riferimenti, contesto, tema, raccolte, e le immagini — la copertina dell'opera e il ritratto dell'autore, con la loro licenza e il loro credito. La lista per intero, in ordine, e' la «lista di chiusura» del punto 8 di `CATALOGO.md`, e le regole sulle immagini stanno ai punti 8-bis (copertina) e 8-quater (ritratto). Se anche una sola voce non si riesce a chiudere, non si pubblica «in attesa di completare»: si scarta e si scrive il motivo in `LOG.md`. **L'unica eccezione e' l'immagine che non esiste** — un testo breve senza edizione propria, un autore senza foto libera su Commons: li' il vuoto e' la risposta giusta e va annotato, mentre una copertina di un'altra edizione o una foto di licenza ignota sono informazioni false su un sito che promette informazioni verificate.
4. **Ogni progresso, fatto o da fare, va tracciato.** Non fidarsi della sola memoria conversazionale: usare `LOG.md` per i lotti di citazioni e questo file per lo stato generale del progetto. Aggiornare la sezione Roadmap qui sotto ogni volta che si completa o si scopre qualcosa, senza aspettare che l'utente chieda "aggiornami".
...
Plath, Simone Weil portati anch'essi a **4 citazioni ciascuno**. Dodicesimo
lotto: Goliarda Sapienza, Wisława Szymborska, Antonio Tabucchi, Natalia
Ginzburg, Charles Dickens portati anch'essi a **4 citazioni ciascuno**.
Archivio a 251 autori, 621 citazioni, 46 con una sola citazione, 83 con tre o
