...
<style>
  ...
  .mine-print-hint[hidden] { display: none; }
...
<div class="page-main" role="main">
  ...
  <div class="mine-empty sans" id="mineEmpty">
    ...
    anche fra un mese.</p>
    <p>Quando ne hai raccolte un po&#39;, da questa pagina le stampi <strong>tutte insieme in un foglio
    solo</strong> &mdash; o le salvi in PDF: citazioni, fonti e le tue note, come un&#39;antologia fatta
    da te.</p>
  ...
  <div class="mine-tools sans" id="mineTools" hidden>
    ...
    <button class="js-only" type="button" id="printBtn">Stampa tutto in un foglio</button>
  </div>
  <p class="mine-print-hint sans js-only" id="minePrintHint" hidden>
...
(function () {
  ...
  var printHintEl = document.getElementById('minePrintHint');
  ...
  function refreshCount() {
    ...
    if (n === 0) {
      ...
      toolsEl.hidden = true;
      if (printHintEl) { printHintEl.hidden = true; }
      emptyEl.hidden = false;
    } else {
      countEl.textContent = n === 1 ? '1 citazione sottolineata' : n + ' citazioni sottolineate';
      toolsEl.hidden = false;
      // La stampa unica e' la funzione che nessuno trovava: il pulsante diceva
      // solo "Stampa" e non lasciava capire che raccoglie tutto in un documento
      // unico. La riga sotto lo spiega, e nomina il PDF — che e' la parola che
      // la gente ha in testa quando vuole tenersi qualcosa.
      if (printHintEl) { printHintEl.hidden = false; }
      emptyEl.hidden = true;
    }
  }
